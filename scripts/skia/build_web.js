...
const toolsScriptPath = path.join(ROOT_PATH, 'third_party', 'vendor_tools', 'Tools.sh');
spawnSync('bash', ['-c', `source ${toolsScriptPath}`], { stdio: 'inherit' });

const skiaSyncScriptPath = path.join(ROOT_PATH, 'third_party', 'skia', 'tools', 'git-sync-deps');
spawnSync('python3', [skiaSyncScriptPath], { stdio: 'inherit' });

function buildArch(arch) {
    const OUT_REAL_PATH = path.join(ROOT_PATH, 'third_party', 'out', 'skia', 'web', arch);
    const args = [
    ...
    const gnResult = spawnSync('bin/gn', ['gen', OUT_REAL_PATH, `--args=${argString}`], {
        stdio: 'inherit',
    });

    if (gnResult.status !== 0) {
        throw new Error(`Failed to generate build files for ${arch}`);
    }

    const ninjaResult = spawnSync('ninja', ['-C', OUT_REAL_PATH], {
        stdio: 'inherit',
    });

    if (ninjaResult.status !== 0) {
        throw new Error(`Failed to build for ${arch}`);
    }
}
